...
//Global variables
static char timestamp[64], combined_stat[MAX_STATLEN+2000], current_stat[MAX_STATLEN], current_stat_top[MAX_TOPSTATLEN], reuse_stat[MAX_STATLEN], *ip, *mname, d_filter[200], e_filter[200];
static int n_partitions = 0, n_ethconnections = 0, stat_interval = 1, send_interval = 5, true = 1, PORT = 0, current_column=13, file_handle=0, file_column_limit = 250, d_f_pos=-1, e_f_pos=-1, n_top=5, c_ops=-1;
...
void *get_top()
{
	...
	char buffer[1024], top_stat[MAX_TOPSTATLEN], mname[200], ttimestamp[64];
	int line = 1, marker = 0;
	mname[200] = '\0';
	...
	if (file)
	{
		while(true)
		{
			...
			while (fgets(buffer, 1024, file))
			{
				...
				{
					if(contains(buffer, " S ")||contains(buffer, " R "))
					{
						if(c_ops){
...
void custom_strcat(char *reuse_stat, char *mname, const char *variable, double value)
{
	char tmp[512];
	sprintf(tmp, "%s.%s %lf %s\n", mname, variable, value, timestamp);
	strcat(reuse_stat, tmp);
}

void custom_strcat_float(char *reuse_stat, char *mname, const char *variable, float value)
{
	char tmp[512];
	sprintf(tmp, "%s.%s %.2f %s\n", mname, variable, value, timestamp);
	strcat(reuse_stat, tmp);
}
...
void *get_stat_remote()
{
	...
	while(true)
	{
		...
		custom_strcat_float((char *)&reuse_stat, mname, "loadavg.1min", ldavgstat.onemin);
		custom_strcat_float((char *)&reuse_stat, mname, "loadavg.5min", ldavgstat.fvmin);
		custom_strcat_float((char *)&reuse_stat, mname, "loadavg.15min", ldavgstat.ftnmin);
		custom_strcat_float((char *)&reuse_stat, mname, "cpu.user", (comstat.user / overalltotal) * 100);
		custom_strcat_float((char *)&reuse_stat, mname, "cpu.sys", (comstat.system / overalltotal) * 100);
		custom_strcat_float((char *)&reuse_stat, mname, "cpu.idle", (comstat.idle / overalltotal) * 100);
		custom_strcat_float((char *)&reuse_stat, mname, "cpu.nice", (comstat.nice / overalltotal) * 100);
		custom_strcat_float((char *)&reuse_stat, mname, "cpu.iowait", (comstat.iowait / overalltotal) * 100);
		custom_strcat_float((char *)&reuse_stat, mname, "cpu.total", ((comstat.user + comstat.system + comstat.nice + comstat.iowait) / overalltotal) * 100);
		custom_strcat_float((char *)&reuse_stat, mname, "sys.intr", comstat.intr);
		custom_strcat_float((char *)&reuse_stat, mname, "sys.ctxt", comstat.ctxt);
		custom_strcat_float((char *)&reuse_stat, mname, "sys.procrun", cstats.procr);
		custom_strcat_float((char *)&reuse_stat, mname, "sys.procblc", cstats.procb);
		custom_strcat((char *)&reuse_stat, mname, "memory.used", mstats.used);
		custom_strcat((char *)&reuse_stat, mname, "memory.free", mstats.free);
		custom_strcat((char *)&reuse_stat, mname, "memory.total", mstats.total);
		custom_strcat((char *)&reuse_stat, mname, "memory.swap", mstats.swapc);
		custom_strcat((char *)&reuse_stat, mname, "memory.buffer", mstats.buffer);
		custom_strcat((char *)&reuse_stat, mname, "memory.cached", mstats.cached);

		for(x = 0; x<n_partitions; x++)
		{
			sprintf(vname, "disk.%s.readsPerSec", dstats[x].dname);
			custom_strcat_float((char *)&reuse_stat, mname, vname, (dstats[x].readc - odstats[x].readc));
			sprintf(vname, "disk.%s.timeTakenPerReadMS", dstats[x].dname);
			custom_strcat_float((char *)&reuse_stat, mname, vname, ((dstats[x].readt - odstats[x].readt))/(float)(dstats[x].readc - odstats[x].readc));
			sprintf(vname, "disk.%s.readsKBPerSec", dstats[x].dname);
			custom_strcat_float((char *)&reuse_stat, mname, vname, ((dstats[x].reads - odstats[x].reads)*512)/1024.0);
			sprintf(vname, "disk.%s.writesPerSec", dstats[x].dname);
			custom_strcat_float((char *)&reuse_stat, mname, vname, (dstats[x].writec - odstats[x].writec));
			sprintf(vname, "disk.%s.timeTakenPerWriteMS", dstats[x].dname);
			custom_strcat_float((char *)&reuse_stat, mname, vname, ((dstats[x].writet - odstats[x].writet))/(float)(dstats[x].writec - odstats[x].writec));
			sprintf(vname, "disk.%s.writesKBPerSec", dstats[x].dname);
			custom_strcat_float((char *)&reuse_stat, mname, vname, ((dstats[x].writes - odstats[x].writes)*512)/1024.0);
			sprintf(vname, "disk.%s.IOInprogress", dstats[x].dname);
			custom_strcat_float((char *)&reuse_stat, mname, vname, dstats[x].ioip - odstats[x].ioip);
			sprintf(vname, "disk.%s.avgRequestSize", dstats[x].dname);
			custom_strcat_float((char *)&reuse_stat, mname, vname, (float)((dstats[x].reads - odstats[x].reads) + (dstats[x].writes - odstats[x].writes))/ (float)((dstats[x].readc - odstats[x].readc) + (dstats[x].writec - odstats[x].writec)));
			sprintf(vname, "disk.%s.avgQueueSize", dstats[x].dname);
			custom_strcat_float((char *)&reuse_stat, mname, vname, (float)(dstats[x].iowt - odstats[x].iowt) / (float)(stat_interval * 1000));
			sprintf(vname, "disk.%s.percentUtil", dstats[x].dname);
			custom_strcat_float((char *)&reuse_stat, mname, vname, ((dstats[x].iot - odstats[x].iot) * 100) / (float)(stat_interval * 1000));
			sprintf(vname, "disk.%s.weightedTimeOnIO", dstats[x].dname);
			custom_strcat_float((char *)&reuse_stat, mname, vname, dstats[x].iowt - odstats[x].iowt);
		}

		for(x = 0; x<n_ethconnections; x++)
		{
			sprintf(vname, "network.%s.bytesRecived", nstats[x].ename);
			custom_strcat((char *)&reuse_stat, mname, vname, nstats[x].brecived - onstats[x].brecived);
			sprintf(vname, "network.%s.bytesTransmit", nstats[x].ename);
			custom_strcat((char *)&reuse_stat, mname, vname, nstats[x].btransmit - onstats[x].btransmit);
			sprintf(vname, "network.%s.packetsRecived", nstats[x].ename);
			custom_strcat((char *)&reuse_stat, mname, vname, nstats[x].precived);
			sprintf(vname, "network.%s.packetsTransmit", nstats[x].ename);
			custom_strcat((char *)&reuse_stat, mname, vname, nstats[x].ptransmit);
		}
		custom_strcat((char *)&reuse_stat, mname, "network.activeconopens", nstats[0].activeopens - onstats[0].activeopens);
		custom_strcat((char *)&reuse_stat, mname, "network.passiveconopens", nstats[0].passiveopens - onstats[0].passiveopens);
		custom_strcat((char *)&reuse_stat, mname, "network.attemptfails", nstats[0].attemptfails - onstats[0].attemptfails);
		custom_strcat((char *)&reuse_stat, mname, "network.establishresets", nstats[0].estabresets - onstats[0].estabresets);
		custom_strcat((char *)&reuse_stat, mname, "network.currentestablish", nstats[0].currentestab);
		custom_strcat((char *)&reuse_stat, mname, "network.retranssegs", nstats[0].retransegs - onstats[0].retransegs);
		custom_strcat((char *)&reuse_stat, mname, "network.inerrors", nstats[0].inerrors - onstats[0].inerrors);
		custom_strcat((char *)&reuse_stat, mname, "network.outresets", nstats[0].outresets - onstats[0].outresets);

		strcpy(current_stat, replace_all(reuse_stat, "-nan", "0.00"));
		strcpy(current_stat, replace_all(reuse_stat, "nan", "0.00"));
		ocstats = cstats;
		memcpy(&odstats, &dstats, sizeof(dstats));
		memcpy(&onstats, &nstats, sizeof(nstats));
...
// This will return the current system stat
char *getcurrent_stats()
{
	strcpy(combined_stat, current_stat);
	strcat(combined_stat, current_stat_top);
	//printf("%s\n", combined_stat);
	return combined_stat;
}

// This will return current stat's length
int getcurrent_stat_length()
{
	return strlen(combined_stat);
}
...
int main(int argc, char *argv[])
{
	...
	{
		printf("Unable to Start Top Stat Collection Thread!\n");
		return -1;
	}
	if(contains(argv[1],"-H")||contains(argv[1],"-h")||argc == 1){
		help();
		return -1;
	}else if(contains(argv[1],"-p")||contains(argv[1],"-P")){
		c_ops=0;
		...
		if( argc>=6)
		{
			printf("Streaming data to ip : %s on port %d with the interval %d ...\n>>>>> 2%% of CPU overhead will be there since it's using 'nc' for streaming <<<<<\n", ip, PORT, send_interval);
			...
			while (true)
			{
				char transferStat[MAX_STATLEN];
				sprintf(transferStat, "printf \"%s\" | nc %s %d\n", getcurrent_stats(), ip, PORT);
				FILE *file = popen(transferStat, "r");
				pclose(file);
				sleep(send_interval);
			}
		}else{
		...
		c_ops=2;
		printf("Saving stats to local with interval of %d ...\n", send_interval);
