...
#
include < stdio.h > #include < stdlib.h > #include < string.h > #include < unistd.h > #include < time.h > #include < linux / version.h > #include < sys / sysinfo.h > #include < sys / socket.h > #include < netinet / in .h > #include < netdb.h >

#define MAX_STATLEN 100000# define MAX_TOPSTATLEN 5000# define MAX_PARTITIONS 150# define MAX_ETHERNETCON 50
...
char * replace_all(char * source, char * orig, char * rep) {
	...
	while (p != NULL) {
		outStr = realloc(outStr, (strlen(source) + replen + 1)); * outStr = '\0';
	...
	if (file) {
		...
		while (fgets(buffer, MAX_STATLEN, file)) {
			if (contains(buffer, "cpu ")) {
				sscanf(buffer, cpu_scan[0], & cpustat - > user, & cpustat - > nice, & cpustat - > system, & cpustat - > idle, & cpustat - > iowait, & cpustat - > irq, & cpustat - > softirq, & cpustat - > steal, & cpustat - > guest, & cpustat - > guestnice);
